...
import { ChevronRight, Upload, X, CheckCircle, Camera, MapPin, Tag, FileText, DollarSign } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
        <div className="bg-white border border-gray-100 rounded-2xl p-10 max-w-md w-full text-center shadow-lg">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Ad Posted Successfully!</h2>
          <p className="text-gray-500 mb-2">Your ad is now live and visible to buyers across Sri Lanka.</p>
          <p className="text-sm text-gray-400 mb-8">Ad reference: #WA{Math.floor(Math.random() * 90000) + 10000}</p>
          <div className="flex flex-col gap-3">
            <Link to="/" className="bg-[#1a237e] hover:bg-[#283593] text-white font-bold py-3 rounded-xl transition-colors">
              Back to Home
            </Link>
            <button onClick={() => { setSubmitted(false); setStep(0); setForm({ category: '', subcategory: '', title: '', description: '', price: '', negotiable: false, district: '', location: '', name: '', phone: '', images: [] }); }}
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Post Your Free Ad</h1>
              ...
              <div key={s} className="flex items-center flex-1">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                    ...
                    i === step ? 'bg-[#1a237e] text-white' :
                    'bg-gray-100 text-gray-400'
                  }`}>
                    {i < step ? '✓' : i + 1}
                  </div>
                  <span className={`text-xs font-medium hidden sm:block ${i === step ? 'text-[#1a237e]' : i < step ? 'text-green-600' : 'text-gray-400'}`}>
                    {s}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-2 ${i < step ? 'bg-green-400' : 'bg-gray-100'}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6">
          ...
          {step === 0 && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-5">Choose the best category for your ad</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map(cat => (
                  <button
                    key={cat.id}
                    onClick={() => { update('category', cat.name); update('subcategory', ''); }}
                  ...
                  >
                    ...
                    <p className="text-sm font-semibold text-gray-800 mt-1">{cat.name}</p>
                    <p className="text-xs text-gray-400">{cat.count.toLocaleString()} ads</p>
                  </button>
                ))}
              </div>
              ...
              {form.category && selectedCat && (
                <div className="mt-5">
                  <p className="text-sm font-semibold text-gray-700 mb-3">Select Subcategory</p>
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-5">Provide accurate details to attract buyers</p>
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Ad Title <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    value={form.title}
          ...
          {step === 2 && (
            <div>
              <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
                <DollarSign size={20} className="text-[#1a237e]" /> Price & Location
              </h2>
              <p className="text-sm text-gray-500 mb-5">Set your price and location</p>
              <div className="space-y-5">
                <div>
                  ...
                  <label className="flex items-center gap-2 mt-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-gray-700">Price is negotiable / Contact for price</span>
                  </label>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    <MapPin size={14} className="inline mr-1 text-[#1a237e]" /> District <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={form.district}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">City / Town <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    value={form.location}
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-5">Ads with photos get 10x more views. Add up to 10 images.</p>
              ...
              <div className="mt-5 bg-blue-50 border border-blue-100 rounded-xl p-4">
                <p className="text-sm font-semibold text-[#1a237e] mb-2">📸 Photo Tips</p>
          ...
          {step === 4 && (
            <div>
              <h2 className="text-lg font-bold text-gray-900 mb-1">Review Your Ad</h2>
              <p className="text-sm text-gray-500 mb-5">Please confirm the details before posting</p>

              <div className="space-y-4">
                {[
                  { label: 'Category', value: form.category ? `${form.category}${form.subcategory ? ' › ' + form.subcategory : ''}` : 'Not selected' },
                  { label: 'Title', value: form.title || 'Not provided' },
                  { label: 'Description', value: form.description ? form.description.substring(0, 100) + (form.description.length > 100 ? '...' : '') : 'Not provided' },
                  { label: 'Price', value: form.negotiable ? 'Negotiable' : form.price ? `Rs. ${Number(form.price).toLocaleString()}` : 'Not set' },
                  { label: 'Location', value: form.location && form.district ? `${form.location}, ${form.district}` : 'Not provided' },
                  { label: 'Contact', value: form.name && form.phone ? `${form.name} – ${form.phone}` : 'Not provided' },
                ].map(item => (
                  <div key={item.label} className="flex gap-4 py-3 border-b border-gray-50 last:border-0">
                    ...
                    <span className="text-sm font-medium text-gray-900 flex-1">{item.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                <p className="text-sm text-yellow-800">
                  By posting this ad, you agree to our <Link to="/terms" className="underline">Terms of Use</Link> and confirm that your listing complies with our guidelines.
                </p>
              </div>
            </div>
          )}
